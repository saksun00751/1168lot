      ...
      <div className="p-4">
        ...
        {item.result ? (
          ...
          <div className="text-center py-1.5 mb-3">
            <div className="text-[22px] font-bold text-ap-primary tabular-nums tracking-wide">{item.countdown}</div>
            <div className="text-[10px] text-ap-tertiary mt-0.5">ชม. · นาที · วินาที</div>
          </div>
        )}
        ...
        {item.isOpen ? (
          <Link
            href={item.href}
            className="block w-full text-center bg-ap-blue text-white rounded-full py-2 text-[12px] font-semibold hover:bg-ap-blue-h transition-colors"
          >
            แทงหวย →
          </Link>
        ) : (
