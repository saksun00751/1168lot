...
export default function BetClassicForm({ lotteryFlag, bills, numberLimits, onAddBills }: Props) {
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  const inputCls    = "w-full text-center text-[13px] font-bold text-ap-primary outline-none bg-transparent tabular-nums py-1.5";
  const disabledCls = "bg-[#f0f0f0]";

  return (
    <div className="p-0">
      {toast && (
        <div className="mx-4 mt-3 px-3 py-2 rounded-xl bg-yellow-50 border border-yellow-200 text-[12px] text-yellow-700 font-semibold">
          ⚠ {toast}
        </div>
      )}
      ...
      <div className="px-3 pb-2 pt-3 overflow-x-auto">
        <table className="w-full border-collapse text-[12px]" style={{ minWidth: 340 }}>
          <thead>
            <tr className="bg-[#4b5563] text-white">
              <th className="border border-[#6b7280] px-2 py-2 text-center font-bold w-[28%]">
                <div>หมายเลข</div>
                <button onClick={handleReverse} className="text-[10px] text-yellow-300 underline mt-0.5 hover:text-yellow-100">
                  (กลับเลข)
                </button>
              </th>
              <th className="border border-[#6b7280] px-2 py-2 text-center font-bold w-[21%]">
                <div>บน</div>
                <button onClick={() => handleCopyCol("top")} className="text-[10px] text-yellow-300 underline mt-0.5 hover:text-yellow-100">
                  (คัดลอก)
                </button>
              </th>
              <th className="border border-[#6b7280] px-2 py-2 text-center font-bold w-[21%]">
                <div>ล่าง</div>
                <button onClick={() => handleCopyCol("bot")} className="text-[10px] text-yellow-300 underline mt-0.5 hover:text-yellow-100">
                  (คัดลอก)
                </button>
              </th>
              <th className="border border-[#6b7280] px-2 py-2 text-center font-bold w-[21%]">
                <div>โต๊ด</div>
                <button onClick={() => handleCopyCol("tod")} className="text-[10px] text-yellow-300 underline mt-0.5 hover:text-yellow-100">
                  (คัดลอก)
                </button>
              </th>
              <th className="border border-[#6b7280] px-2 py-2 text-center font-bold w-[9%]">ลบ</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => {
              ...
              return (
                <tr key={row.id} className="bg-white hover:bg-gray-50">
                  <td className="border border-[#d1d5db] px-2 py-1 text-center font-bold text-ap-primary tabular-nums">
                    {row.number}
                  </td>
                  {/* บน — เปิดทุก row */}
                  <td className="border border-[#d1d5db] p-0">
                    <input type="text" inputMode="numeric" value={row.top}
                  ...
                  {/* ล่าง — เปิดเฉพาะ 2 หลัก */}
                  <td className={`border border-[#d1d5db] p-0 ${!is2 ? disabledCls : ""}`}>
                    {is2 && <input type="text" inputMode="numeric" value={row.bot}
                      onChange={(e) => updateRow(row.id, "bot", e.target.value)}
                      className={inputCls} />}
                  </td>
                  {/* โต๊ด — เปิดเฉพาะ 3 หลัก */}
                  <td className={`border border-[#d1d5db] p-0 ${is2 ? disabledCls : ""}`}>
                    {!is2 && <input type="text" inputMode="numeric" value={row.tod}
                      onChange={(e) => updateRow(row.id, "tod", e.target.value)}
                      className={inputCls} />}
                  </td>
                  <td className="border border-[#d1d5db] px-1 py-1 text-center">
                    <button onClick={() => deleteRow(row.id)}
                      className="text-ap-red hover:text-red-700 text-[16px] leading-none">🗑</button>
                  </td>
                </tr>
              );
            })}
            ...
            <tr className="bg-white">
              <td className="border border-[#d1d5db] p-0">
                <input ref={inputRef} type="text" inputMode="numeric"
                  ...
                  maxLength={3}
                  className="w-full text-center text-[13px] font-bold text-ap-primary outline-none bg-transparent tabular-nums py-2 placeholder:text-ap-tertiary placeholder:font-normal" />
              </td>
              <td className="border border-[#d1d5db] bg-[#f9fafb]" />
              <td className="border border-[#d1d5db] bg-[#f9fafb]" />
              <td className="border border-[#d1d5db] bg-[#f9fafb]" />
              <td className="border border-[#d1d5db] bg-[#f9fafb]" />
            </tr>
      ...
      <div className="px-4 pb-3 pt-1 flex items-center gap-2">
        ...
        <input type="text" value={note} onChange={(e) => setNote(e.target.value)}
          className="flex-1 border border-ap-border rounded-lg px-3 py-1.5 text-[12px] outline-none focus:border-ap-blue bg-white" />
      ...
      <div className="px-4 pb-4 flex gap-2">
        <button onClick={clearAll}
          className="flex-1 py-2 rounded-xl border-2 border-ap-border text-[13px] font-bold text-ap-secondary hover:bg-ap-bg active:scale-95 transition-all">
          ล้างทั้งหมด
        </button>
        <button onClick={handleSubmit} disabled={!canSubmit}
          className={[
            "flex-2 px-8 py-2 rounded-xl text-[13px] font-bold transition-all active:scale-95",
            canSubmit
